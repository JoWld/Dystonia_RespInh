function el=ELPH_EyelinkInitDefaults(window, BGCol, FGCol)

%%% Von Eyelink-Demo übernommen (leicht verändert)
%%% Initiiert die Eyelink-Default-Werte (unter anderem auch die Möglichkeit zur
%%% Darstellung des Auges auf dem Subject-Monitor während der Kalibration

% Initialize eyelink defaults and control code structure
switch nargin
    case 0
        error('no window pointer given');
    case 1
        BGCol = floor([255 255 255]/2);
        FGCol = [255 255 255];
    case 2
        FGCol = [255 255 255];
end
...
el.foregroundcolour = FGCol;
el.msgfontcolour    = FGCol;
el.imgtitlecolour   = FGCol;
...
el.feedbackbeep=0; %nachträglich von Christian geadded
...
el.calibrationtargetcolour=FGCol;

el.devicenumber = []; %nachträglich von Christian geadded
...
el.eyeimgsize=60; % percentage of screen
el.helptext='';
